...
//#########################################################################################PARSER SECTION
  this.parse_gen = (tokens) => {
    ...
    this.pointer_pacg = (ref) => {
	  ...
	  ref.mem_location.push({type:"pointer_addr",expr:[]});
    ...
    this.factor_pacg = (ref) => { // ussually for pushing stuff like integers and stuff from identifiers to the stack to be handled later
      if (tokens[this.index][1] == 1) {//int lit
		ref.expr.push({type:"int_lit",name: tokens[this.index][2]});
        ...
        this.index++;
		  ...
		  if(tokens[this.index+1][2] != "("){//identifier direct acess parse
            ...
            while(tokens[this.index][2] != ")"){
				...
				ref.expr[ref.expr.length-1].parameters.push({expr:[]});
                this.expr_pacg(ref.expr[ref.expr.length-1].parameters[temporary_index_for_poping_stuff_off_the_stack-1]);
                if(tokens[this.index][2] == ","){
                    this.index++;
                }
            }
        ...
        ref.expr.push({type:"identifier_mem_loc",name: tokens[this.index+1][2]});
		  this.match("$");


		 this.index++;
	  } else if (tokens[this.index][2] == "*") {
		 ref.expr.push({type:"pointer_read",mem_location:[]});
         this.pointer_pacg(ref.expr[ref.expr.length-1]);

      }else if(tokens[this.index][2] == "arg"){
		    ref.expr.push({type:"arg_read",expr:[]});
            this.match("arg");
            this.match("[");
       ...
       ADJP 900
          ...
          */
			  ...
			  ref.expr.push({type:"bpa_expr",expr:[]});
            this.match("bpa");
            this.match("[");
      ...
      while (["*", "/", "<<", ">>","%","^","|","&"].includes(tokens[this.index][2])) {
		...
		if (tokens[this.index][2] == "*") {
          ...
          this.factor_pacg(ref);
		        ...
		        ref.expr.push({type:"binary_2",name: op_buf});
      ...
      while (tokens[this.index][2] == "+" || tokens[this.index][2] == "-") {
		...
		if (tokens[this.index][2] == "+") {
          ...
          this.term_pacg(ref);
				...
				ref.expr.push({type:"binary_1",name: op_buf});
    ...
    this.if_pacg = (ref) => {
      ...
      this.match("if");
	  ref.block.push({type:"if",block:[],condition:"",expr_1:{expr:[]},expr_2:{expr:[]},cond_ins:""});
    ...
    this.block_pacg = (ref) => {
      ...
      while (tokens[this.index][2] != "}") {
        if (tokens[this.index][2] == "*") {
		  ref.block.push({type:"pointer_write",mem_location:[],value:{expr:[]},cond_ins:""});
		  ...
		  ref.block.push({type:"while",block:[],condition:"",expr_1:{expr:[]},expr_2:{expr:[]},cond_ins:""});
          this.block_pacg(ref.block[ref.block.length-1]);
          ...
          this.index = index_buffer_after;
            ...
            this.match("asm");
			ref.block.push({type:"asm_blk",contents:tokens[this.index][2]});

            this.index++;
                
        } else if (tokens[this.index][2] == "return") {
		 ref.block.push({type:"return",expr:[]});
          this.match("return");
          this.expr_pacg(ref.block[ref.block.length-1]);

          this.match(";");
        }else if(tokens[this.index][1] == 0 && tokens[this.index+1][2] == "("){
			ref.block.push({type:"function_call",name:tokens[this.index][2],parameters:[]});
            let token_id_call_buff = tokens[this.index][2];
            this.index++;
            this.match("(");
            let temporary_index_for_poping_stuff_off_the_stack = 0;
            while(tokens[this.index][2] != ")"){
                temporary_index_for_poping_stuff_off_the_stack++;
				ref.block[ref.block.length-1].parameters.push({expr:[]});
    ...
    this.main_pacg = () => {
     this.ast.program.push({type:"function",name:"main",block:[]});
	 this.ast.sym_table["main"] = {type:"function",name:"main"};

      this.match("int");
      this.match("main");
      this.match("(");
      this.match(")");
      this.block_pacg(this.ast.program[this.ast.program.length-1]);
    ...
    this.func_pacg = () =>{
        while(this.index<tokens.length&& tokens[this.index][2] == "int"){  
            this.match("int");
			this.ast.program.push({type:"function",name:tokens[this.index][2],block:[]});
	        this.ast.sym_table[tokens[this.index][2]] = {type:"function",name:tokens[this.index][2]};
  ...
  this.ast_to_asm =(ast)=>{
	  ...
	  let bool_gen = (current_bool) =>{
		  expr_gen(current_bool.expr_1.expr);
		  expr_gen(current_bool.expr_2.expr);
		  expt(current_bool.cond_ins);
	  }
	  
	  
	  let expr_gen = (current_expr) =>{
		  let pc_expr = 0;
		  ...
		  while(pc_expr < current_expr.length){
			  ...
			  if(type == "int_lit"){
				  ...
				  expr_gen(current_expr[pc_expr].mem_location[0].expr);
				  expt("IPOP");
				  expt("LDIA");
				  expt("PUSH");
				...
				expt("PUSH");
				  ...
				  for(i=0; i<current_expr[pc_expr].parameters.length;i++){
					  expr_gen(current_expr[pc_expr].parameters[i].expr);
				  }
	  ...
	  let blk_gen= (current_blk) =>{
		  for(let pc_blk=0;pc_blk<current_blk.length;pc_blk++ ){
			  if(current_blk[pc_blk].type == "pointer_write"){
					expr_gen(current_blk[pc_blk].mem_location[0].expr);
					expt("IIPOP");
					expr_gen(current_blk[pc_blk].value.expr);
					expt("POP");
					expt("STIIA");
			  }else if(current_blk[pc_blk].type == "bpa_write"){
				  expt("BPTOII");
				  expt("IIPUSH");
				  expr_gen(current_blk[pc_blk].offset.expr);
				  expt("SUB");
				  expt("IIPOP");
				  expr_gen(current_blk[pc_blk].value.expr);
				  expt("POP");
				  ...
				  blk_gen(current_blk[pc_blk].block);
				  ...
				  for(i=0; i<current_blk[pc_blk].parameters.length;i++){
					  expr_gen(current_blk[pc_blk].parameters[i].expr);
				  }
